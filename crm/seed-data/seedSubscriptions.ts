...
export const seedSubscriptions = async (context: KeystoneContext) => {
    ...
    for (const subscription of subscriptions) {
        ...
        const nameSub: string =
            templateSub.find((item) => item.id === subscription.subscriptionId)?.name || 'Английский индивидуальный';

        const emailStudent = clients.find((item) => item.id === subscription.userId)?.email;
