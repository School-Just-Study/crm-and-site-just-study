...
const { PAYTURE_URL, PAYTURE_TERMINAL_PASSWORD, PAYTURE_TERMINAL_USD } = require('../config');
...
exports.paytureEnStatus = (orderId) =>
    new Promise((resolve, reject) => {
        inPayEn.payStatus(orderId, function (error, response, body, responseObject) {
            if (error) reject(error);
...
exports.paytureEnRefund = (orderId) =>
    new Promise((resolve, reject) => {
        inPayEn.refund({ OrderId: orderId }, function (error, response, body, responseObject) {
            if (error) reject(error);
