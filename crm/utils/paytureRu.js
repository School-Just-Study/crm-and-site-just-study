...
const { PAYTURE_URL, PAYTURE_TERMINAL_RUB, PAYTURE_TERMINAL_PASSWORD } = require('../config');
...
exports.paytureRuStatus = (orderId) =>
    new Promise((resolve, reject) => {
        inPayRu.payStatus(orderId, function (error, response, body, responseObject) {
            if (error) reject(error);
...
exports.paytureRuRefund = (orderId) =>
    new Promise((resolve, reject) => {
        inPayRu.refund({ OrderId: orderId }, function (error, response, body, responseObject) {
            if (error) reject(error);
